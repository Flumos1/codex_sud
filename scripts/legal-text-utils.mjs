...
export function classifyOutcome(text) {
  ...
  const rules = [
    ...
    // Remand is more specific than the generic "передати справу" transfer, so it must be checked first.
    ["remanded", 0.86, /(?:направити|передати)\s+.{0,80}нов[а-яіїєґ]*\s+розгляд[а-яіїєґ]*/u],
